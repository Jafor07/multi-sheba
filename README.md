...
- **Chatbot** — bottom-right floating widget on every page. This is a **rule-based keyword matcher against your FAQ and service list**, not a real AI chatbot — it costs nothing to run. It answers common questions and links straight to the matching order page; anything it doesn't recognize gets a "ask on WhatsApp" button. If you later want it to actually understand free-form questions, that means calling an LLM API per message, which has a real ongoing cost — worth doing once you have paying customers, not before.
